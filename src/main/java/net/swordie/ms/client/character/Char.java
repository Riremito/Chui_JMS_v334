...
public class Char {
	...
	@OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
	private Inventory equipInventory = new Inventory(EQUIP, 96/*52*/);

	@JoinColumn(name = "consumeInventory")
	@OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
	private Inventory consumeInventory = new Inventory(InvType.CONSUME, 96/*52*/);

	@JoinColumn(name = "etcInventory")
	@OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
	private Inventory etcInventory = new Inventory(InvType.ETC, 96/*52*/);

	@JoinColumn(name = "installInventory")
	@OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
	private Inventory installInventory = new Inventory(InvType.INSTALL, 96/*52*/);
	...
	private final List<FuncKeyMap> funcKeyMaps;
	...
	private final Map<Integer, Long> skillCoolTimes;
	@ElementCollection
	...
	@Transient
	private final CharacterPotentialMan potentialMan;
	...
	@Transient
	private final GachaponManager gachaponManager;
	@Transient
    ...
    @Transient
    private final Map<Integer, Integer> currentDirectionNode;
	@Transient
	...
	@Transient
	private final Map<Integer, PsychicArea> psychicAreas;
	@Transient
	private final Map<Integer, PsychicLock> psychicLocks;
	@Transient
	private final Map<Integer, PsychicLockBall> psychicLockBalls;
	@Transient
...
//		outPacket.encodeByte(0); // unk, not in kmst
		...
		if (mask.isInMask(DBChar.Money)) {
			...
			outPacket.encodeInt(0/*getId()*/);
			outPacket.encodeLong(0); // 小鋼珠
			outPacket.encodeInt(0); // 楓葉點數
			outPacket.encodeLong(0);
			outPacket.encodeByte(true/*false*/);
			outPacket.encodeInt(0);
		}
...
/*		if (mask.isInMask(DBChar.ItemSlotConsume) || mask.isInMask(DBChar.ShopBuyLimit)) {
		...
		if (mask.isInMask(DBChar.ItemSlotEquip)) {
			...
			outPacket.encodeShort(0); //10 技能造型 -1700 ~ -1800
...
/*		if (mask.isInMask(DBChar.ItemSlotCash)) {
		...
		outPacket.encodeInt(0); // size-> 兩個decodebuffer[8]
...
/*			for (int i = 0; i < 13; i++) {
		...
		outPacket.encodeShort(0);
		for (int i = 0; i < 10; i++) {
			outPacket.encodeByte(0); // 如果 > 0 需要 additeminfo
		}
...
//				outPacket.encodeInt(0);
		...
		if (mask.isInMask(DBChar.ChosenSkills)) {
			if (JobConstants.isPhantom(getAvatarData().getCharacterStat().getJob())) {
				for (int i = 1; i <= 4/*5*/; i++) { //Shifted by +1 to accomodate the Skill Management Tabs
...
/*		if (mask.isInMask(DBChar.CoreAura)) {
		...
		if (mask.isInMask(DBChar.RedLeafInfo)) {
			...
			outPacket.encodeInt(getId());
			outPacket.encodeInt(5/*0*/);
			outPacket.encodeInt(0);
			outPacket.encodeArr(new byte[40]);
		}

		outPacket.encodeArr(new byte[24]);
	}
	...
	public void warp(Field toField, Portal portal, boolean characterData, boolean saveReturnMap) {
		...
		getClient().write(Stage.setField(this, toField, getClient().getChannel(), true/*false*/, 0, characterData, hasBuffProtector(),
