...
public class Stage {
    ...
    public static OutPacket setField(Char chr, Field field, int channelId, boolean ibara/*dev*/, int oldDriverID,
        ...
        short shortSize1 = 0;
        outPacket.encodeShort(shortSize1);
        for (int i = 1; i <= shortSize1; i++) {
            outPacket.encodeInt(i);
            outPacket.encodeInt(0);
        }
        short shortSize2 = 1;
        outPacket.encodeShort(shortSize2);
        if (shortSize2 > 0) {
            for (int i = 1; i <= shortSize2; i++) {
                outPacket.encodeInt(i);
                outPacket.encodeInt(i/*0*/);
            }
            outPacket.encodeByte(ibara); // いばら伺服器開關
            outPacket.encodeInt(9000/*0*/); // 技能冷卻時間?
        }
        outPacket.encodeInt(channelId - 1); // Damn nexon, randomly switching between starting at 1 and 0...
        outPacket.encodeByte(0);
        outPacket.encodeByte(0);
        outPacket.encodeByte(ibara/*dev*/); // いばら介面開關
...
/*            boolean bool = false;
        ...
        outPacket.encodeArr(new byte[6/*1*/]);
