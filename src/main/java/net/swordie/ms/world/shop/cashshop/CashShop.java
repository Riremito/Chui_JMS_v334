...
//        }
        ...
        size = 0;
        outPacket.encodeShort/*Int*/(size); // randomItemCount
        for (int i = 0; i < size; i++) {
            outPacket.encodeInt(0); // sn
            outPacket.encodeLong(0); // item flag
            outPacket.encodeLong(0); // item flag2
...
//        // buffer aBest, 3 inner loops (int (category) + int (gender) + int (sn))
        ...
        outPacket.encodeShort(size);
        for (int i = 0; i < size; i++) {
            outPacket.encodeArr(new byte[128]);
